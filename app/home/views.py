...
def login():
    ...
    if "user_id" in session:        # 如果已经登录，则直接跳转到首页
        return redirect(url_for("home.index"))  #/app/templates/home/index.html
    form = LoginForm()              # 实例化LoginForm类 forms.py
    if form.validate_on_submit():   # 如果提交
        data = form.data            # 接收表单数据
        # 判断验证码
        if session.get('image').lower() != form.verify_code.data.lower():   # 将输入的验证码和session中保存的验证码字符串都转换成小写后再作判断
            flash('验证码错误',"err")
...
def index():
    ...
    all_devices = Goods.query.filter_by(right=0).order_by(
                    Goods.addtime.desc()
                        ).all()
    page = request.args.get('page', 1, type=int)  # 获取page参数值
    all_page_data = Goods.query.filter_by(right=0).paginate(page=page, per_page=12)
    device_cart = Cart.query.filter_by(user_id=int(user_id)).all()
    my_cart = Cart.query.filter_by(user_id = int(user_id)).order_by(Cart.addtime.desc()).all()
    return render_template('home/index.html',all_devices=all_devices,device_cart=device_cart,all_page_data=all_page_data,my_cart=my_cart) # 渲染模板

@home.route("/devices_list/<int:supercat_id>/")
def devices_list(supercat_id=None):  # supercat_id 为设备大分类ID
    """
    设备页
    """
    page = request.args.get('page', 1, type=int)  # 获取page参数值
    ...
    user_id = session.get('user_id',0)
    my_cart = Cart.query.filter_by(user_id = int(user_id)).order_by(Cart.addtime.desc()).all()
    return render_template('home/devices_list.html',page_data=page_data,hot_goods=hot_goods,supercat_id=supercat_id,my_cart=my_cart)

@home.route("/devices_detail/<int:id>/")
def devices_detail(id=None):  # id 为设备ID
    """
    详情页
    """
    ...
    return render_template('home/devices_detail.html',goods=goods,hot_goods=hot_goods,similar_goods=similar_goods,user_id=user_id,my_cart=my_cart)   # 渲染模板
...
def devices_search():
    ...
    my_cart = Cart.query.filter_by(user_id = int(user_id)).order_by(Cart.addtime.desc()).all()
    return render_template("home/devices_search.html", page_data=page_data,keywords=keywords,hot_goods=hot_goods,my_cart=my_cart)
...
def cart_add():
    ...
    db.session.commit()   # 提交数据
    return redirect(url_for('home.device_cart'))
...
@home.route("/device_cart/")
@user_login
def device_cart():
    user_id = session.get('user_id',0)
    cart = Cart.query.filter_by(user_id = int(user_id)).order_by(Cart.addtime.desc()).all()
    if cart:
        return render_template('home/device_cart.html',cart=cart)
    else:

...
def goods_add():
    ...
    form.subcat_id.choices = [(v.id, v.cat_name) for v in SubCat.query.filter_by(super_cat_id=supercat_list[0][0]).all()]  # 为sub_cat_id添加属性
    #form.current_price.data = form.data['original_price'] # 为current_pirce 赋值
    if form.validate_on_submit():    # 添加设备情况
        data = form.data
        goods = Goods(
            name = data["name"],
...
def goods_edit(id=None):
    ...
    if request.method == "GET":
        ...
        form.right.data = goods.right
        form.device_ip.data = goods.device_ip
        form.username.data = goods.username
        form.password.data = goods.password
        ...
        goods.right = form.data["right"],
        goods.device_ip = form.data["device_ip"],
        goods.username = form.data["username"],
        goods.password = form.data["password"],
        db.session.add(goods)  # 添加数据
        db.session.commit()     # 提交数据
        return redirect(url_for('admin.index')) # 页面跳转
...
def subcat_del():
    ...
    if request.method == 'POST':
        ...
        for id in cat_ids:
            ...
            if count:
                return "<script>alert('该分类下有设备，请先删除分类下的设备');history.go(-1);</script>"
        # 使用in_ 方式批量删除，需要设置synchronize_session为False,而 in 操作估计还不支持
        # 解决办法就是删除时不进行同步，然后再让 session 里的所有实体都过期
